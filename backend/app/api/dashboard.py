...
async def get_summary(
    ...
    async def _sum_revenue(since):
        stmt = select(func.coalesce(func.sum(ImpressionEvent.revenue), 0.0)).where(ImpressionEvent.created_at >= since)
        if publisher_id:
...
async def get_trend(
    ...
    return {"trend": [{"date": r.day, "impressions": r.impressions, "revenue": float(r.revenue)} for r in result.all()]}