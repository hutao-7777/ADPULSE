...
async def get_ad_unit_config(
    ...
    if not au:
        return SdkAdUnitConfig(
            ad_unit_id=str(ad_unit_id), ad_format="banner",
    ...
    for s in sources:
        ...
        entry = WaterfallEntry(
            source_id=str(s.id), network_name=net_name,
            ecpm=s.ecpm, priority=s.priority,
        )
        if s.bidding_endpoint:
            bidding.append(BiddingEntry(
                source_id=str(s.id), network_name=net_name,
    ...
    return SdkAdUnitConfig(
        ...
        fallback_html="<div style=\"background:#eee;padding:20px;text-align:center\">Ad</div>",
