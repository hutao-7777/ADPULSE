...
if TYPE_CHECKING:
    from app.models.publisher import AdUnit
...
class AdNetwork(Base):
    ...
    supports_bidding: Mapped[bool] = mapped_column(
    ...
    ad_sources: Mapped[List["AdSource"]] = relationship(
        "AdSource", back_populates="ad_network"
    )
...
class AdSource(Base):
    ...
    ad_unit_id: Mapped[uuid.UUID] = mapped_column(
        Uuid(as_uuid=True),
        ForeignKey("ad_units.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ad_network_id: Mapped[uuid.UUID] = mapped_column(
        Uuid(as_uuid=True),
        ForeignKey("ad_networks.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    ad_network: Mapped["AdNetwork"] = relationship(
        "AdNetwork", back_populates="ad_sources"
    )
    __table_args__ = (
        Index("ix_ad_sources_unit_network", "ad_unit_id", "ad_network_id"),
    )
...
class ImpressionEvent(Base):
    ...
    ad_source_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        Uuid(as_uuid=True),
        ForeignKey("ad_sources.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
