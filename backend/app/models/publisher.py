...
class App(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    publisher_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("publishers.id", ondelete="CASCADE"), nullable=False, index=True)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    ...
    __table_args__ = (Index("ix_apps_publisher_status", "publisher_id", "status"), Index("ix_apps_platform", "platform"))
...
class AdUnit(Base):
    ...
    ad_sources: Mapped[List["AdSource"]] = relationship("AdSource", back_populates="ad_unit")

    __table_args__ = (Index("ix_ad_units_app_status", "app_id", "status"), Index("ix_ad_units_format", "ad_format"))
