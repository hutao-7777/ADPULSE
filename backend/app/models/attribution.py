...
class ConversionEvent(Base):
    ...
    click_id: Mapped[Optional[str]] = mapped_column(String(255), nullable=True, index=True)
    ...
    attributed_network: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    attribution_model: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    metadata_: Mapped[dict] = mapped_column("metadata", JSON, default=dict, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, nullable=False)

    touchpoints: Mapped[List["Touchpoint"]] = relationship("Touchpoint", back_populates="conversion_event")
    attribution_results: Mapped[List["AttributionResult"]] = relationship("AttributionResult", back_populates="conversion_event")
...
class Touchpoint(Base):
    ...
    conversion_event_id: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid(as_uuid=True), ForeignKey("conversion_events.id", ondelete="SET NULL"), nullable=True, index=True)
    cost: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    metadata_: Mapped[dict] = mapped_column("metadata", JSON, default=dict, nullable=False)

    conversion_event: Mapped[Optional["ConversionEvent"]] = relationship("ConversionEvent", back_populates="touchpoints")

    __table_args__ = (
        Index("ix_touchpoints_user_adunit_seq", "user_id", "ad_unit_id", "touchpoint_seq"),
...
class AttributionResult(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    conversion_event_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("conversion_events.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    conversion_event: Mapped["ConversionEvent"] = relationship("ConversionEvent", back_populates="attribution_results")

    __table_args__ = (Index("ix_attribution_results_conversion_model", "conversion_event_id", "model_type"),)
...
class TrafficQualityScore(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    ad_unit_id: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid(as_uuid=True), ForeignKey("ad_units.id", ondelete="CASCADE"), nullable=True, index=True)
