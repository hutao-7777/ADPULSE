...
def _ensure_security_settings():
    if not settings.SECRET_KEY or settings.SECRET_KEY in {"change-me-in-production", "dev-secret-change-me", ""}:
...
async def lifespan(app: FastAPI):
    ...
    reset_on_start = os.environ.get("RESET_DB_ON_START", "").lower() in {"1", "true", "yes"}
