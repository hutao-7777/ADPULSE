...
async def login(request: LoginRequest, db: AsyncSession = Depends(get_db), _rate_limit: bool = Depends(login_rate_limiter)) -> dict:
...
async def register(
    ...
    db: AsyncSession = Depends(get_db),
    _rate_limit: bool = Depends(register_rate_limiter),
) -> dict:
