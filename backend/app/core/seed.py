...
async def _seed_auth(session):
    ...
    await session.execute(
        role_permissions.insert().values(role_id=admin_role.id, permission_id=perm1.id)
    )
    await session.commit()
...
async def _seed_publisher_data(session):
    ...
    sources = [
        AdSource(
            ad_unit_id=banner.id,
            ad_network_id=_uuid("net_admob"),
            instance_name="AdMob Main",
            ecpm=4.50,
            priority=1,
        ),
        AdSource(
            ad_unit_id=banner.id,
            ad_network_id=_uuid("net_meta"),
            instance_name="Meta Bid",
            ecpm=5.20,
            priority=1,
            bidding_endpoint="https://meta.com/bid",
        ),
        AdSource(
            ad_unit_id=banner.id,
            ad_network_id=_uuid("net_unity"),
            instance_name="Unity Backup",
            ecpm=3.20,
            priority=2,
        ),
        AdSource(
            ad_unit_id=interstitial.id,
            ad_network_id=_uuid("net_admob"),
            instance_name="AdMob Interstitial",
            ecpm=6.00,
            priority=1,
        ),
    ]
